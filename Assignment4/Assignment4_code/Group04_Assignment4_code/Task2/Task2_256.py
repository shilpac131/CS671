...
encoded = Dense(256, activation='tanh',kernel_initializer=initializer)(inputs)
...
train_loss1 = autoencoder_256.evaluate(train_data, train_data)
...
val_pred = autoencoder_256.predict(val_data)
...
reconstructed_images_1 = autoencoder_256.predict(test_images)
reconstructed_images_2 = autoencoder_256.predict(val_images)
reconstructed_images_3 = autoencoder_256.predict(train_images)
...
# # Autoencoder 256  - 3 Hidden layer 
...
encoded = Dense(256, activation='tanh',kernel_initializer=initializer)(encoded)
...
callback = EarlyStopping(monitor="val_loss",min_delta=0.0001, verbose = 0, restore_best_weights=True, patience = 5)
# Train the model
autoencoder_256_3.fit(train_data, train_data, validation_data=(val_data, val_data), epochs=100000, batch_size=32,callbacks = callback, shuffle = True)
...
val_pred = autoencoder_256_3.predict(val_data)
val_reconstruction_error = keras.metrics.mean_squared_error(val_data, val_pred).numpy().mean()
print('Average reconstruction error on validation data:', val_reconstruction_error)
...
train_loss2 = autoencoder_256_3.evaluate(train_data, train_data)
print('Average reconstruction error on training data:', train_loss2)
...
# Reconstruct images for each architecture
reconstructed_images_1 = autoencoder_256_3.predict(test_images)
reconstructed_images_2 = autoencoder_256_3.predict(val_images)
reconstructed_images_3 = autoencoder_256_3.predict(train_images)
