...
autoencoder_128_train = Dense(128, activation='tanh',kernel_initializer=initializer)(inputs)

# Define decoding layer
decoded = Dense(784, activation='tanh',kernel_initializer=initializer)(autoencoder_128_train)
...
compressed_model_128 = Model(inputs=inputs, outputs=autoencoder_128_train)
...
compressed_128_train_data = compressed_model_128.predict(train_data)
compressed_128_test_data = compressed_model_128.predict(test_data)
compressed_128_val_data = compressed_model_128.predict(val_data)
...
# Train the model
model_fit = model.fit(compressed_128_train_data, train_label, validation_data=(compressed_128_val_data, val_label), epochs=10000, batch_size=32,callbacks=callback,shuffle = True)

# Evaluate the model on the validation set
val_loss, val_acc = model.evaluate(compressed_128_val_data, val_label)
...
test_loss, test_acc = model.evaluate(compressed_128_test_data, test_label)
...
predictions = model.predict(compressed_128_test_data, verbose=1)
...
encoded_128_3 = Dense(128, activation='tanh',kernel_initializer=initializer)(encoded)

# Define decoding layers 
decoded = Dense(400, activation='tanh',kernel_initializer=initializer)(encoded_128_3)
...
callback = EarlyStopping(monitor="val_loss",min_delta=0.0001, verbose = 0, restore_best_weights=True, patience = 5)
# Train the model
autoencoder_128_3.fit(train_data, train_data, validation_data=(val_data, val_data), epochs=10000, batch_size=32,callbacks = callback, shuffle = True)
...
# Create a new model with the input as the original input and output as the output of the compressed layer
compressed_model_128_3 = Model(inputs=inputs, outputs=encoded_128_3)
...
# Use the compressed model to extract the compressed representation of the training data
compressed_128_3_train_data = compressed_model_128_3.predict(train_data)
compressed_128_3_test_data = compressed_model_128_3.predict(test_data)
compressed_128_3_val_data = compressed_model_128_3.predict(val_data)
...
callback = EarlyStopping(monitor="loss",min_delta=0.0001, verbose = 0, restore_best_weights=True, patience = 2)
# Train the model
model_fit = model.fit(compressed_128_3_train_data, train_label, validation_data=(compressed_128_3_val_data, val_label), epochs=10000, batch_size=32,callbacks=callback,shuffle = True)

# Evaluate the model on the validation set
val_loss, val_acc = model.evaluate(compressed_128_3_val_data, val_label)
print('Validation accuracy:', val_acc)
...
predictions = model.predict(compressed_128_3_test_data, verbose=1)
p_class = np.argmax(predictions, axis=1)
# cm = tf.math.confusion_matrix(test_label, p_class)
cm = confusion_matrix(test_label, p_class)
cm_display = ConfusionMatrixDisplay(cm)
...
test_loss, test_acc = model.evaluate(compressed_128_3_test_data, test_label)
print('test accuracy:', test_acc)
